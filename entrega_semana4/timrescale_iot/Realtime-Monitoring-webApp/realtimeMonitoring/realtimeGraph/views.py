...
def get_map_json_team_eight_couple_two(request, **kwargs):
    data_result = {}
    ...
    try:
        mintemp = float(request.GET.get("mintemp", 35))
    except:
        mintemp = 35
    ...
    for location in locations:
        ...
        avgVal = locationData.aggregate(Avg("avg_value"))["avg_value__avg"]
        data.append({
            'locations': {'country': f'{location.city.name}, ',
                          'state': f'{location.state.name}, ',
                          'city': f'{location.country.name}',
                          },
            'population': stations.count(),
            'temperature': f'{round(avgVal if avgVal != None else 0, 2)} celcius',
        })
